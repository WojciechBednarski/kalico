...
syncemitter_alloc(char name[16], int alloc_stepcompress, int high_precision)
{
    ...
    if (!alloc_stepcompress)
        return se;
    se->sc = high_precision ? stepcompress_hp_alloc(&se->msg_queue)
                            : stepcompress_alloc(&se->msg_queue);
...
steppersync_alloc_syncemitter(struct steppersync *ss, char name[16]
                              , int alloc_stepcompress, int high_precision)
{
