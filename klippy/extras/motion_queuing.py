...
class PrinterMotionQueuing:
    ...
    def allocate_syncemitter(
        self, mcu, name, alloc_stepcompress=True, high_precision=False
    ):
        name = name.encode("utf-8")[:15]
        ss = self._lookup_steppersync(mcu)
        ffi_main, ffi_lib = chelper.get_ffi()
        se = ffi_lib.steppersync_alloc_syncemitter(
            ss, name, alloc_stepcompress, high_precision
        )
